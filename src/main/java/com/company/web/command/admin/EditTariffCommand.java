...
public class EditTariffCommand extends Command {
    ...
    private static final Logger logger = LogManager.getLogger(EditTariffCommand.class);
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String[] values2 = request.getParameterValues("descriptions");
        ...
        for (int a=0;a<values.length;a++){
            ...
            tariffInfo.setTariffName(values[a]);
            tariffInfo.setDescription(values2[a]);
            tariffInfoMap.put(Language.values()[Integer.parseInt(langs[a])],tariffInfo);
        }

        try {
            tariffDao.updateTariffAndTariffInfo(tariff,tariffInfoMap);
